...
- **Skill(sdd-next)** (this skill) - Identifies next tasks and creates execution plans
...
2. **Codebase Context Gathering**: If codebase documentation has been generated, automatically gathers task-relevant context (via `Skill(doc-query)`) including:
...
**Availability**: Requires codebase documentation generated by the `Skill(code-doc)` skill. Invoke `Skill(doc-query)` to access these capabilities.
...
If available, invoke `Skill(doc-query)` which provides commands like:
...
### Skill(doc-query) - Smart Context for SDD Workflows

**Availability**: Invoke `Skill(doc-query)` to access codebase documentation query capabilities (which use `sdd doc` commands under the hood).
...
**Important**: All workflows in this guide that reference `Skill(doc-query)` are **recommendations for enhanced context**, not requirements. The core sdd-next workflow functions without them.

## When to Use This Skill

Use `Skill(sdd-next)` when:
...
- Updating task status or progress (use `Skill(sdd-update)`)
...
```
Have a spec already?
├─ No → Use `Skill(sdd-plan)` first to create one
└─ Yes → Continue below

What do you need to do?
├─ Find next task to work on → Use `Skill(sdd-next)` (this skill)
├─ Create execution plan → Use `Skill(sdd-next)` (this skill)
├─ Update task status → Use `Skill(sdd-update)`
├─ Journal a decision → Use `Skill(sdd-update)`
├─ Actually write code → Use implementation tools (after Developer creates plan)
└─ Create new spec → Use `Skill(sdd-plan)`
...
└─ Task complete → Use `Skill(sdd-update)` to update status
```
...
If codebase documentation has been generated, use `Skill(doc-query)` for rapid context gathering:
...
If documentation is available, invoke `Skill(doc-query)` to:
...
- **Spec file corrupt/invalid**: Validate spec with `validate-spec` command or regenerate with Skill(sdd-plan)
...
If codebase documentation has been generated, use `Skill(doc-query)` for:
...
Use `Skill(sdd-next)` skill to identify the next actionable task
...
**User Approves:**
- Mark task as in_progress (handoff to `Skill(sdd-update)`)
...
4. Document blocker with `Skill(sdd-update)`
...
4. Document deviation with `Skill(sdd-update)`
5. Revise plan with `Skill(sdd-next)`
...
**Solution:** Regenerate JSON spec file with `Skill(sdd-plan)`
...
**Integration Points:**
- Reads specs created by `Skill(sdd-plan)`
- Updates status using `Skill(sdd-update)`
...
**Skill(sdd-update)** - Use alongside this skill to:
...
*For creating specifications, use `Skill(sdd-plan)`. For tracking progress, use `Skill(sdd-update)`.*
