...
This skill uses the standardized `ai_tools` module (`claude_skills.common.ai_tools`) for all external AI tool consultations. The module provides:
...
- **Parallel execution** - Run multiple AI tools concurrently for comprehensive analysis
- **Robust error handling** - Automatic timeout, retry, and fallback logic
- **Tool availability detection** - Check which AI CLIs are installed before consultation

**For detailed API documentation, see:** [AI Tools API Reference](../../docs/API_AI_TOOLS.md)

**Key functions used by this skill:**
- `execute_tool()` - Single AI tool consultation
- `execute_tools_parallel()` - Multi-agent consultation (runs tools in parallel)
...
The `sdd test consult` command internally uses these functions to route failure types to appropriate AI tools and handle responses consistently.